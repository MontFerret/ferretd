...
func TestCancellationAndHookFailureRetainBothCauses(t *testing.T) {
	for _, stage := range []string{"compile", "run", "debug"} {
		t.Run(stage, func(t *testing.T) {
			...
			option := ferret.WithBeforeRunHook(func(context.Context) (context.Context, error) {
				cancel()
			...
			if stage == "compile" {
				option = ferret.WithBeforeCompileHook(func(context.Context) error {
					cancel()
...
func TestCanceledAdmissionDoesNotApplyOptionsOrPublish(t *testing.T) {
	...
	cancel()
	calls := 0
	planOption := func(api.PlanOptions) error { calls++; return nil }
	sessionOption := func(api.SessionOptions) error { calls++; return nil }
...
func TestRuntimeRunPreservesOutputAndAllCleanupErrors(t *testing.T) {
	...
	engine, err := ferret.New(
		ferret.WithSessionCloseHook(func() error { sessions.Add(1); return sessionErr }),
		ferret.WithPlanCloseHook(func() error { plans.Add(1); return planErr }),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOptionsReachNativeOwnerOnceInOrder(t *testing.T) {
	runtime := newTestRuntime(t)
	...
	session, err := plan.NewSession(t.Context(), nil, func(target api.SessionOptions) error {
		calls++

		return ferret.WithSessionParam("value", 1)(target)
	}, api.WithParam("value", 2), func(target api.SessionOptions) error {
		calls++

		return ferret.WithSessionRuntimeParam("value", ferretruntime.NewInt(3))(target)
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || calls != 2 || string(output.Content) != "3" {
		t.Fatalf("output=%q error=%v calls=%d", output.Content, err, calls)
	}
}
...
func TestCancellationDuringRunHookSurvivesContextReplacement(t *testing.T) {
	...
	engine, err := ferret.New(ferret.WithBeforeRunHook(func(context.Context) (context.Context, error) {
		cancel()
...
func TestDebugCloseRetainsNativeCleanupResult(t *testing.T) {
	...
	engine, err := ferret.New(ferret.WithSessionCloseHook(func() error { return cause }))
	if err != nil {
		t.Fatal(err)
	}
